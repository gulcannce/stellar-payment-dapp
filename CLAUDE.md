...
- **Level 4 (25 Temmuz 2026'da tamamlandı, RiseIn'e henüz submit edilmedi — submission adımı kullanıcının onayını bekliyor):** Onaylanan Idea Submission'a (Cross-Border Freelancer Payment & Invoice Tracker) dayanan bir Invoice Tracker MVP'si. `contracts/invoice` (multi-invoice, tek deploy — adres: `CD6FLY7IQ2J2ZI5E6OJC37D44A6PHYAGX7WX3KHY5F2JHIYWNMK47NKI`, `deployer` identity'siyle deploy edildi) + `useInvoiceContract`/`useInvoiceEvents` hook'ları + `InvoiceForm`/`InvoiceCard`/`InvoiceList` component'leri + in-app Friendbot fonlama butonu (`useFriendbot`/`friendbot.js`) + Vercel Analytics (`@vercel/analytics`, temel sayfa görüntüleme takibi — custom event'ler kodda var ama Hobby planda toplanmıyor, bkz. aşağıdaki "ÖNEMLİ DÜZELTME" notu) + Google Form feedback linki (config.js'teki `FEEDBACK_FORM_URL`) eklendi. Uygulama **Vercel'de canlı**: https://stellar-payment-dapp-beta.vercel.app (proje: `gulcanncee-gmailcoms-projects` scope'unda `stellar-payment-dapp`, `npx vercel deploy --prod --scope gulcanncee-gmailcoms-projects` ile yeniden deploy edilebilir) — GitHub Pages de hâlâ ayrı olarak canlı (Level 1-3 kanıtı). **Tamamlanan gereksinimler (25 Temmuz 2026, stellar.expert'te canlı doğrulandı):** 10 farklı gerçek cüzdan `create_invoice` çağırdı (liste README'nin "Proof of 10+ User Wallet Interactions" bölümünde — GA4W…U3NI, GC6Q…25KJ, GDIW…SX27, GBWP…ES6X, GDNX…HPNO, GD5R…Z6YK, GCZS…47TJ, GCJ7…JAJZ, GBPI…O4JG, GBBX…ZDV4); geri bildirim formunda 12 gerçek yanıt (10/11'i 5/5, 1/11 4/5 puan); Level 4'e özel 4 ekran görüntüsü (`screenshots/level4-*.jpg`) çekildi ve README'ye eklendi; demo video Drive'da. **Kalan tek adım:** kullanıcının RiseIn submission sayfasında GitHub'ı bağlayıp submit etmesi — bu, hesap/OAuth gerektiren ve geri alınamayan bir adım olduğu için Claude tarafından otomatik yapılmıyor, kullanıcının kendisi yapmalı (ya da açıkça "submit et" derse birlikte adım adım yapılabilir).

**KRİTİK HATA DÜZELTMESİ (26 Temmuz 2026, `useInvoiceContract.js`):** `fromRawInvoice`'daki durum decode mantığı yanlıştı — `scValToNative`, veri taşımayan `InvoiceStatus` enum varyantlarını (`Draft`/`Sent`/...) `{tag: "Draft"}` değil, tek elemanlı bir dizi (`["Draft"]`) olarak döndürüyor. Eski kod (`raw.status?.tag ?? raw.status`) bunu yakalayamıyordu, dizi `.tag`'siz olduğu için `raw.status` (dizinin kendisi) kalıyordu. `InvoiceCard.jsx`'teki `invoice.status === "Draft"` gibi katı karşılaştırmalar bu yüzden **hep false** dönüyordu — sonuç: Level 4 boyunca oluşturulan **15 faturanın hiçbirinde Gönder/Öde/İptal butonu hiç görünmedi**, sadece `create_invoice` gerçekten test edilebildi. Rozet metni (`STATUS_LABEL[invoice.status]`) tesadüfen doğru görünüyordu çünkü obje-anahtarı erişiminde JS diziyi otomatik string'e çeviriyor — bu yüzden hata gözden kaçmıştı. Zincir üzerinde `get_invoice`'ı doğrudan sorgulayarak (`scValToNative` çıktısını inceleyerek) teşhis edildi, düzeltildi (`statusToString` yardımcı fonksiyonu eklendi), 39/39 test ve build geçti, Vercel'e deploy edildi. Eski 15 taslak fatura zincirde bozulmamıştı (sadece client yanlış okuyordu) — düzeltmeyle birlikte artık gönderilebilir/ödenebilirler. **Level 5 için 40+ yeni kullanıcı davet edilmeden önce kullanıcının kendisi Freighter ile create→gönder→öde akışını uçtan uca canlıda test edip doğrulaması gerekiyor** (Freighter onay pop-up'larına Claude tıklayamıyor).

**Level 5 (24 Temmuz 2026'da sadece hazırlık yapıldı, Level 4 tamamlanmadan gerçek anlamda ilerletilemez):** Level 5'in "50+ gerçek kullanıcı" ve "geri bildirime dayalı iyileştirme" gereksinimleri, Level 4'ün 10 kullanıcısı gerçekleşmeden anlamlı şekilde yapılamaz (bkz. yukarıdaki Level 4 notu). Şimdiden hazırlanan, insan bağımlılığı olmayan parçalar: bir pitch deck (Artifact — **şu an private**, README'ye linklendi ama kullanıcı share menüsünden paylaşmadıkça reviewer göremez, bunu unutmayalım), Level 5'e özel bir Google Form (isim + email + wallet adresi + 1-5 puan topluyor: `https://docs.google.com/forms/d/e/1FAIpQLSd1zM7jY7YF9UbcdeMv6QYWieydV8yTK19PzoA3eQDdne27fA/viewform`), ve README'de "🔭 Level 5 — Growth & Product Iteration" başlığı altında doldurulmayı bekleyen bir iskelet (feedback→değişiklik→commit tablosu, Excel export linki placeholder'ı). **Hâlâ insan tarafından yapılması gerekenler:** pitch deck'i paylaşıma açmak (veya PowerPoint'e aktarmak), 50+ gerçek kullanıcı toplamak, yanıtları Excel'e aktarıp README'deki tabloyu gerçek verilerle doldurmak, demo video.

**Auction süresi uzatma notu (25 Temmuz 2026):** v3'ün (`CCIO4FACYBGQJJIPBRPQFJ3UGWSOELLM52YG7BICEBTUHSXN75G7WS25`) süresi dolmuştu (gerçek testçiler bu sırada üzerine teklif vermişti — o geçmiş hâlâ stellar.expert'te kanıt olarak duruyor). Aynı yöntemle **v4** deploy edildi (adres: `CBF6ASW3BJ6JVNBHJV7P2TXU7OPGTD2EM5HZFKGWUJ6A33EBLVXC5VVC`, aynı registry + token, 13 saatlik yeni `end_time`), `config.js` güncellendi, Vercel + gh-pages'e yeniden deploy edildi. Süre tekrar dolarsa aynı adımlar (`make -C contracts/auction build` + `stellar contract deploy` + `initialize` + config.js güncelle + `npm run build` + `npx vercel deploy --prod --scope gulcanncee-gmailcoms-projects` + `npx gh-pages -d dist`) tekrarlanabilir. README'deki "Example Transactions" tx hash'leri önceki instance'lara ait, hâlâ geçerli kanıt olarak duruyor.
...
**Analytics/feedback (Level 4):** `src/App.jsx`'e eklenen `<Analytics />` (`@vercel/analytics/react`) sayfa görüntüleme/ziyaretçi verisi topluyor (dashboard: vercel.com/gulcanncee-gmailcoms-projects/stellar-payment-dapp/analytics), bu Hobby (ücretsiz) planda da çalışıyor. **ÖNEMLİ DÜZELTME (24 Temmuz 2026, canlı dashboard'ta doğrulandı):** `useWallet.js`/`useInvoiceContract.js` içindeki `track()` custom event çağrıları ("wallet_connected"/"invoice_created"/"invoice_paid") kodda duruyor ama **Vercel'in Hobby planında hiç veri toplamıyor** — Analytics sayfasının "Events" sekmesi "No custom events... Upgrade to a Pro team to access this feature" diyor. Yani bu event'ler "10+ kullanıcı etkileşimi kanıtı" için **kullanılamaz** (plan Pro'ya yükseltilmedikçe). Asıl kanıt kaynağı olarak bunun yerine **stellar.expert'teki `contracts/invoice` sayfası** kullanılıyor (`https://stellar.expert/explorer/testnet/contract/CD6FLY7IQ2J2ZI5E6OJC37D44A6PHYAGX7WX3KHY5F2JHIYWNMK47NKI`) — on-chain, ücretsiz, sahte olamaz; her `create_invoice`/`pay_invoice` çağıran farklı adresi tx hash'iyle gösteriyor. README/CLAUDE.md bu yönde güncellendi (Vercel Analytics artık sadece "temel trafik izleme", asıl kullanıcı-etkileşim kanıtı değil). `FeedbackLink.jsx` component'i `FEEDBACK_FORM_URL` dolu olduğu için render oluyor, bu kısımda bir sorun yok.

**Test/gerçek kullanıcı durumu (24 Temmuz 2026):** Vercel Analytics'te "4 visitors" görünüyor ama hepsi Türkiye'den ve muhtemelen kullanıcının kendi cihazları/önceki test oturumları — gerçek/dış test kullanıcısı değil. stellar.expert'te de tek bir `create_invoice` çağrısı var, o da yine geliştirme sırasında kullanılan test cüzdanından (`GB7PA3BQBREDBSVJPVYRUVHYIZAD6EAZDTNPEZ5RRCAEIPRUZDX3H7P7`) geliyor. **Henüz hiç gerçek/dış test kullanıcısı yok** — kullanıcı arkadaşlarına/topluluğa mesaj atıp gerçek testleri başlatmayı planlıyor.
